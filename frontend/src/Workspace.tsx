...
import { api, apiRequest, downloadFile } from './lib/api';

type Props = { section: string; token: string };
...
export function Workspace({ section, token }: Props) {
  if (section === 'Customers') return <Customers token={token}/>;
  if (section === 'Products') return <Products token={token}/>;
  if (section === 'Stock movements') return <Stock token={token}/>;
...
function Stock({token}:{token:string}) {const [rows,setRows]=useState<Product[]>([]);const [search,setSearch]=useState('');const [selected,setSelected]=useState<Product|null>(null);const load=()=>api<{data:Product[]}>(`/api/products?limit=100&search=${encodeURIComponent(search)}`,token).then(r=>setRows(r.data));useEffect(()=>{const timer=window.setTimeout(load,180);return()=>window.clearTimeout(timer)},[search]);return <div className="workspace-page"><div className="toolbar"><label className="search toolbar-search"><Search size={16}/><input aria-label="Search stock" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search stock by name or SKU"/></label></div><section className="data-card"><div className="data-card-title"><div><p className="eyebrow">AUDIT TRAIL</p><h2>Stock movements</h2></div><span>{rows.length} products</span></div><div className="data-head"><span>Product</span><span>Current stock</span><span>Minimum</span><span>Action</span></div>{rows.map(r=><div className="data-row" key={r.id}><b>{r.productName}<small>{r.sku}</small></b><span>{r.currentStock} units</span><span>{r.minimumStockAlertQuantity} units</span><button onClick={()=>setSelected(r)}>Receive stock</button></div>)}{rows.length===0&&<p className="muted">No products match “{search}”.</p>}</section>{selected&&<ReceiveStock token={token} product={selected} close={()=>setSelected(null)} saved={()=>{setSelected(null);load()}}/>}</div>}
