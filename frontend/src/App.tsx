...
export function App() {
  ...
  return <main className="shell">
    ...
    <section className="content"><header><div><p className="eyebrow">Good morning, {session.user.name.split(' ')[0]}</p><h1>{currentActive === 'Overview' ? 'Operations at a glance' : currentActive}</h1></div><div className="header-actions"><div className="notification-wrap"><button className="icon-button" aria-label={`Notifications${unreadCount ? ` (${unreadCount} unread)` : ''}`} aria-expanded={showNotifications} onClick={() => setShowNotifications((shown) => !shown)}><Bell size={19}/>{unreadCount > 0 && <i/>}</button>{showNotifications && <NotificationPanel notifications={notifications} unreadCount={unreadCount} token={session.token} close={() => setShowNotifications(false)} refresh={refreshWorkspace} />}</div>{canCreateChallan && <button className="new-button" onClick={() => setShowChallan(true)}>+ New challan</button>}</div></header>
      ...
      {currentActive === 'Overview' && <DashboardView dashboard={dashboard} analytics={analytics} token={session.token} role={session.user.role} onNavigate={setActive} />}
...
function Login({ onSuccess }: { onSuccess: (session: Session) => void }) {
  const testAccounts = [
    { role: 'Admin', email: 'admin@stockflow.test' },
    { role: 'Sales', email: 'sales@stockflow.test' },
    { role: 'Warehouse', email: 'warehouse@stockflow.test' },
    { role: 'Accounts', email: 'accounts@stockflow.test' },
  ];
  ...
  function chooseAccount(nextEmail: string) { setEmail(nextEmail); setPassword('FundsRoom@123'); setError(''); }
  return <main className="login-page"><section className="login-copy"><span className="brand-mark">S</span><p className="eyebrow">WHOLESALE OPERATIONS</p><h1>Every order has a story.<br/>Keep it moving.</h1><p>Stockflow keeps sales, customers, and inventory in one calm, reliable workspace.</p><div className="ornament">01&nbsp;&nbsp; Customer to challan<br/>02&nbsp;&nbsp; Challan to stock movement</div></section><section className="login-panel"><aside className="test-accounts" aria-label="Test account credentials"><div className="test-accounts-heading"><div><p className="eyebrow">TEST ACCESS</p><strong>Choose a portal</strong></div><span>Click to fill</span></div><div className="test-account-grid">{testAccounts.map((account) => <button type="button" key={account.role} onClick={() => chooseAccount(account.email)} className={email === account.email ? 'selected' : ''}><b>{account.role}</b><span>{account.email}</span><small>FundsRoom@123</small></button>)}</div></aside><form onSubmit={submit}><p className="eyebrow">WELCOME BACK</p><h2>Sign in to Stockflow</h2><p className="hint">Select a test account above or enter an assigned account.</p><label>Email<input value={email} onChange={(event) => setEmail(event.target.value)} type="email" autoComplete="username" /></label><label>Password<input value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete="current-password" /></label>{error && <p className="error">{error}</p>}<button className="login-button" disabled={loading}>{loading ? 'Signing in...' : 'Enter workspace'} <ChevronRight size={18}/></button></form></section></main>;
}

function DashboardView(props: { dashboard: Dashboard['data'] | null; analytics: Analytics['data'] | null; token: string; role: Role; onNavigate: (section: string) => void }) {
  if (!props.dashboard) return <p className="loading">Loading the live workspace...</p>;
  if (props.role === 'WAREHOUSE') return <WarehouseDashboard token={props.token} onNavigate={props.onNavigate} />;
  return <DashboardBody {...props} dashboard={props.dashboard} />;
}

function WarehouseDashboard({ token, onNavigate }: { token: string; onNavigate: (section: string) => void }) {
  const [pending, setPending] = useState<Array<{ id: string; productName: string; quantity: number; urgency: string; requestedBy: string }>>([]);
  const [lowStock, setLowStock] = useState<Array<{ id: string; productName: string; sku: string; currentStock: number; minimumStockAlertQuantity: number }>>([]);
  useEffect(() => { Promise.all([api<{ data: typeof pending }>('/api/stock-requests?status=PENDING', token), api<{ data: typeof lowStock }>('/api/products?limit=5&lowStock=true', token)]).then(([requests, products]) => { setPending(requests.data); setLowStock(products.data); }).catch(() => { setPending([]); setLowStock([]); }); }, [token]);
  return <div className="warehouse-dashboard"><div className="metric-grid"><Metric label="Pending requests" value={pending.length} note="from Sales" tone="warm"/><Metric label="Low-stock items" value={lowStock.length} note="need attention"/><Metric label="Warehouse tasks" value={pending.length + lowStock.length} note="in this queue"/></div><div className="board"><section className="table-card"><div className="card-heading"><div><p className="eyebrow">INVENTORY CONTROL</p><h2>Low-stock items</h2></div><button onClick={() => onNavigate('Products')}>View products <ChevronRight size={16}/></button></div><div className="table-head"><span>Product</span><span>SKU</span><span>Stock</span><span>Status</span></div>{lowStock.map((item) => <div className="table-row" key={item.id}><b>{item.productName}</b><span>{item.sku}</span><span>{item.currentStock} units</span><span className="status attention">Low stock</span></div>)}{lowStock.length===0&&<p className="muted">No low-stock items.</p>}</section><section className="followups"><p className="eyebrow">SALES HANDOFFS</p><h2>Pending stock requests</h2>{pending.slice(0,5).map((item) => <div className="followup" key={item.id}><span className="avatar small">{item.productName[0]}</span><div><b>{item.productName}</b><small>{item.quantity} units · {item.requestedBy}</small></div><span className={`status urgency-${item.urgency.toLowerCase()}`}>{item.urgency}</span></div>)}{pending.length===0&&<p className="muted">No pending requests.</p>}<button className="text-button" onClick={() => onNavigate('Stock movements')}>Open request queue</button></section></div><section className="recent warehouse-notes"><div className="card-heading"><div><p className="eyebrow">WAREHOUSE NOTES</p><h2>Keep stock movements traceable</h2></div></div><p className="muted">Every approval, receipt, and adjustment is recorded with a reason in Stock movements.</p><button className="new-button" onClick={() => onNavigate('Stock movements')}>View stock movements</button></section></div>;
}

function DashboardBody({ dashboard, analytics, token, onNavigate }: { dashboard: Dashboard['data']; analytics: Analytics['data'] | null; token: string; onNavigate: (section: string) => void }) {
